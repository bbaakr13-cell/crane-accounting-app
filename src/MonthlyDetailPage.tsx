...
export function MonthlyDetailPage() {
  ...
  async function loadCraneHeaderImage(): Promise<HTMLImageElement | null> {
    try {
      const img = new Image();
      img.src = '/crane-header.png';
      await new Promise<void>((resolve, reject) => {
        img.onload = () => resolve();
        img.onerror = () => reject(new Error('crane-header.png not found'));
      });
      return img;
    } catch {
      return null;
  ...
  async function createPdfCanvas() {
    ...
    const craneImage = await loadCraneHeaderImage();
    ...
    // ===== هيدر احترافي مع صورة الكرين =====
    ctx.fillStyle = NAVY;
    ctx.fillRect(0, 0, PDF_WIDTH, 150);
    ctx.fillStyle = GOLD;
    ctx.fillRect(0, 0, 9, 150);

    // صورة الكرين من public/crane-header.png - وإذا لم توجد يظهر رسم كرين احتياطي
    if (craneImage) {
      const boxX = 20, boxY = 15, boxW = 235, boxH = 120;
      roundedRect(ctx, boxX, boxY, boxW, boxH, 12);
      ctx.save(); ctx.clip();
      const scale = Math.max(boxW / craneImage.width, boxH / craneImage.height);
      const w = craneImage.width * scale, h = craneImage.height * scale;
      ctx.drawImage(craneImage, boxX + (boxW-w)/2, boxY + (boxH-h)/2, w, h);
      ctx.restore();
    } else {
      ctx.strokeStyle = GOLD; ctx.lineWidth = 5;
      ctx.beginPath(); ctx.moveTo(40,112); ctx.lineTo(40,62); ctx.lineTo(175,62); ctx.lineTo(118,105); ctx.stroke();
      ctx.fillStyle = GOLD; ctx.fillRect(28,112,180,10); ctx.fillRect(62,91,70,21);
    }

    drawArabic(ctx, 'الحساب الشهري', 760, 37, 30, '#ffffff');
    drawEnglish(ctx, 'MONTHLY ACCOUNT STATEMENT', 760, 72, 13, '#dce7f2', 800);
    drawArabic(ctx, displayEquipmentName, 760, 101, 18, GOLD);
    drawArabic(ctx, `${monthNames[month]} ${year}`, 760, 128, 14, '#ffffff');

    drawEnglish(ctx, 'BAKR PRO', 275, 34, 23, '#ffffff', 900);
    drawArabic(ctx, 'إدارة حسابات الكرينات', 275, 64, 15, '#ffffff');
    drawArabic(ctx, 'تصميم التطبيق: بكر المسبحي', 275, 91, 12, GOLD);
    drawArabic(ctx, 'خميس مشيط - أبها ومنطقة عسير', 275, 116, 11, '#dce7f2');

    // ===== معلومات فقط في الأعلى - بدون تكرار الإجماليات =====
    const infoY = 165;
    const infoGap = 7;
    const infoWidth = (PDF_WIDTH - 36 - infoGap * 3) / 4;
    drawInfoBox(ctx, 18, infoY, infoWidth, 56, 'المعدة', displayEquipmentName);
    drawInfoBox(ctx, 18 + (infoWidth + infoGap), infoY, infoWidth, 56, 'الشهر', monthNames[month]);
    drawInfoBox(ctx, 18 + (infoWidth + infoGap) * 2, infoY, infoWidth, 56, 'السنة', String(year));
    drawInfoBox(ctx, 18 + (infoWidth + infoGap) * 3, infoY, infoWidth, 56, 'الموقع', 'خميس مشيط - أبها');
    ...
    columnRects.forEach((column) => {
      ctx.fillStyle = NAVY; ctx.fillRect(column.x, tableY, column.width, headerHeight);
      ctx.strokeStyle = '#ffffff33'; ctx.lineWidth = 1; ctx.strokeRect(column.x, tableY, column.width, headerHeight);
    ...
    for (let day = 1; day <= 31; day += 1) {
      ...
      const dateText = day <= daysInMonth ? `${year}-${String(month + 1).padStart(2,'0')}-${String(day).padStart(2,'0')}` : '';
      const values: Record<string,string> = {
        day:String(day), date:dateText, work:row.workType, location:row.tripType,
      ...
      columnRects.forEach((column) => {
        ...
        ctx.strokeStyle=BORDER; ctx.lineWidth=.6; ctx.strokeRect(column.x,rowY,column.width,rowHeight);
        const text=values[column.key]||'';
        if(!text){ drawArabic(ctx,'—',column.x+column.width/2,rowY+rowHeight/2,8,'#94a3b8'); return; }
        const size=fitFontSize(ctx,text,column.width-5,10,6,HACEN_FONT_NAME);
        drawArabic(ctx,text,column.x+column.width/2,rowY+rowHeight/2,size,column.key==='amount'&&combinedExpense>0?RED:TEXT);
      });
    }

    // ===== الإجماليات تحت الجدول فقط =====
    ...
    const summaryItems = [
      ...
      { label:'أيام مسجلة', value:String(totals.registeredDays), color:BLUE, bg:'#eef5ff' },
      { label:'عدد المشاوير', value:String(totals.trips), color:PURPLE, bg:'#f7f2ff' },
    ];
    summaryItems.forEach((item,index)=>{
      const x=18+index*(sumWidth+sumGap);
      roundedRect(ctx,x,summaryY,sumWidth,60,8); ctx.fillStyle=item.bg; ctx.fill(); ctx.strokeStyle=`${item.color}55`; ctx.stroke();
      drawArabic(ctx,item.label,x+sumWidth/2,summaryY+19,12,TEXT);
      drawArabic(ctx,item.value,x+sumWidth/2,summaryY+43,16,item.color);
    });

    const notesY=summaryY+70;
    roundedRect(ctx,18,notesY,PDF_WIDTH-36,50,9); ctx.fillStyle='#fbfdff'; ctx.fill(); ctx.strokeStyle=BORDER; ctx.stroke();
    drawArabic(ctx,'ملاحظات الكشف',PDF_WIDTH-35,notesY+16,13,BLUE);
    drawArabic(ctx,'كشف شهري شامل للأعمال والمشاوير والمصاريف المسجلة للمعدة.',PDF_WIDTH/2,notesY+35,10,MUTED);

    // ===== فوتر قصير =====
    const footerY=1058;
    ctx.fillStyle=NAVY; ctx.fillRect(0,footerY,PDF_WIDTH,PDF_HEIGHT-footerY);
    drawEnglish(ctx,'BAKR PRO',80,footerY+19,17,'#ffffff',900);
    drawArabic(ctx,'إدارة حسابات الكرينات',80,footerY+40,10,'#ffffff');
    drawEnglish(ctx,'0558995962',PDF_WIDTH/2,footerY+22,18,GOLD,900);
    drawArabic(ctx,'خميس مشيط - أبها ومنطقة عسير',PDF_WIDTH/2,footerY+43,10,'#ffffff');
    drawArabic(ctx,'تصميم التطبيق: بكر المسبحي',PDF_WIDTH-92,footerY+20,10,GOLD);
    drawEnglish(ctx,'BAKR PRO',PDF_WIDTH-92,footerY+42,11,'#dce7f2',800);
