...
export function EquipmentDetailPage() {
        ...
        <div className="pt-4">
          ...
          <Card className="p-8 flex flex-col items-center text-center">
            ...
            <button
              ...
              className="mt-4 px-5 py-2.5 rounded-xl bg-gold-500/15 text-gold-300 font-semibold text-sm active:scale-95 transition-transform"
            >
              العودة للمعدات
            </button>
          </Card>
  ...
  /*
    أهم تعديل:
    عند الضغط على الحساب الشهري
    نرسل اسم المعدة إلى صفحة الحساب الشهري.
  */
  function openMonthlyAccount() {
    navigate(
      `/monthly?equipment=${encodeURIComponent(
        equipment.name
      )}`
    );
  }
  ...
  const actionButtons = [
    ...
    },
    {
      label: 'الحساب الشهري',
      icon: CalendarClock,
      onClick: openMonthlyAccount,
      tone:
        'bg-receivable/10 text-receivable border-receivable/20',
    },
    {
      label: 'عرض الحركات',
      icon: Receipt,
    ...
    <AppLayout showHeader={false} showBottomNav={false}>
      <div className="pt-4">
        ...
        <section className="grid grid-cols-3 gap-2.5 mb-4">
          <StatCard
            label="قيمة الأعمال"
            amount={formatSAR(stats.totalJobValue)}
            icon={TrendingUp}
            tone="income"
            delay={0}
          />
          ...
          <StatCard
            label="المصروفات"
            amount={formatSAR(stats.totalExpenses)}
            icon={TrendingDown}
            tone="expense"
            delay={60}
          />
          ...
          <StatCard
            label="صافي الربح"
            amount={formatSAR(stats.netProfit)}
            icon={Wallet}
            tone="profit"
            delay={120}
          />
        ...
        <div className="grid grid-cols-2 gap-2.5 mb-4">
          <Card className="p-3 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-receivable/10 flex items-center justify-center flex-shrink-0">
              <Receipt
                className="w-4 h-4 text-receivable"
                strokeWidth={2}
              />
            </div>
            ...
            <div>
              ...
              <p className="text-sm font-bold text-receivable tabular-nums">
                {formatSAR(stats.totalRemaining)}
              </p>
            </div>
          </Card>
          ...
          {[
            ...
            },
            {
              ...
              value: equipment.capacity
                ? `${equipment.capacity} طن`
                : '—',
            },
            {
              icon: Truck,
              label: 'النوع',
              value: typeLabels[equipment.type],
            },
          ].map((row) => {
            const Icon = row.icon;
          ...
          <Card className="p-6 max-w-sm w-full">
            <div className="flex flex-col items-center text-center">
              <div className="w-14 h-14 rounded-2xl bg-expense/10 flex items-center justify-center mb-4">
                <AlertTriangle
                  className="w-7 h-7 text-expense"
                  strokeWidth={2}
                />
              </div>
              ...
              <p className="text-sm text-slate-400 mb-6">
                هل أنت متأكد من حذف "{equipment.name}"؟
                سيتم حذف المعدة ولكن ستبقى الحركات المالية
                محفوظة.
              </p>
              ...
              <div className="flex gap-3 w-full">
                <button
                  ...
                  disabled={deleting}
                  className="flex-1 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-200 font-medium text-sm active:scale-95 transition-transform"
                >
                  إلغاء
                </button>
