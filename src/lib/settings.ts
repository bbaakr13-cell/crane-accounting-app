...
export async function fetchSettings(): Promise<AppSettings> {
  ...
  return {
    ...
    logo: data.logo_data ?? '',
    defaultPaymentMethod: data.default_payment_method ?? DEFAULT_SETTINGS.defaultPaymentMethod,
    compactMode: data.compact_mode ?? DEFAULT_SETTINGS.compactMode,
    animations: data.animations ?? DEFAULT_SETTINGS.animations,
    showOfflineBadge: data.show_offline_badge ?? DEFAULT_SETTINGS.showOfflineBadge,
    printPhone: data.print_phone ?? DEFAULT_SETTINGS.printPhone,
    reportTitle: data.report_title ?? DEFAULT_SETTINGS.reportTitle,
  };
}
...
export async function saveSettings(s: AppSettings): Promise<void> {
  const { error } = await supabase.from('settings').upsert({
    ...
    currency: s.currency,
    logo_data: s.logo,
    default_payment_method: s.defaultPaymentMethod,
    compact_mode: s.compactMode,
    animations: s.animations,
    show_offline_badge: s.showOfflineBadge,
