...
function categoryFromStorageKey(
  key: string
) {
  const k =
...
function archiveRemovedItem(
  storageKey: string,
  payload: unknown,
  originalIndex: number
) {
  const category =
    categoryFromStorageKey(
      storageKey
    );
