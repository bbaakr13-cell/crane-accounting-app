...
async function buildMergedPdf(equipmentName: string, docs: EquipmentDocument[]) {
  // Uses jsPDF, which is already used by BAAKR PRO. No pdf-lib dependency.
  const pdf = new jsPDF({ orientation: 'portrait', unit: 'pt', format: 'a4', compress: true });
  const pageW = pdf.internal.pageSize.getWidth();
  const pageH = pdf.internal.pageSize.getHeight();

  const coverDataUrl = createCoverImage(equipmentName, docs);
  pdf.addImage(coverDataUrl, 'PNG', 0, 0, pageW, pageH, undefined, 'FAST');

  for (const doc of docs) {
    pdf.addPage();

    // Browser jsPDF cannot import existing PDF pages by itself. Keep the app build-safe
    // and show a clear placeholder page for uploaded PDF documents.
    if (doc.mimeType === 'application/pdf' || doc.fileName.toLowerCase().endsWith('.pdf')) {
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(20);
      pdf.text('PDF DOCUMENT', pageW / 2, 110, { align: 'center' });
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(12);
      pdf.text(doc.fileName || doc.title, pageW / 2, 145, { align: 'center', maxWidth: pageW - 80 });
      pdf.setFontSize(10);
      pdf.text('Stored in BAAKR PRO documents', pageW / 2, 175, { align: 'center' });
      continue;
    }

    try {
      const format = doc.mimeType.includes('png') ? 'PNG' : 'JPEG';
      const img = new Image();
      img.src = doc.dataUrl;
      ...
      pdf.addImage(doc.dataUrl, format, (pageW - w) / 2, (pageH - h) / 2, w, h, undefined, 'FAST');
    } catch (e) {
