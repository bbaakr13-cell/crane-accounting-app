                ...
                {data.items.map((item, index) => {
                  ...
                  return (
                    <div key={index} style={workRow}>
                      ...
                      <div><label style={miniLabel}>البيان</label><input style={inputStyle} value={item.description} onChange={(e) => updateItem(index, 'description', e.target.value)} placeholder="اكتب البيان" /></div>
                      <div><label style={miniLabel}>الكمية</label><input inputMode="decimal" style={inputStyle} value={item.qty === 0 ? '' : String(item.qty)} onChange={(e) => { const v = e.target.value.replace(/[٠-٩]/g, d => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d))).replace(/[^\d.]/g, ''); updateItem(index, 'qty', v === '' ? 0 : Number(v)); }} /></div>
                      <div><label style={miniLabel}>سعر الوحدة</label><input inputMode="decimal" style={inputStyle} value={item.unitPrice === 0 ? '' : String(item.unitPrice)} onChange={(e) => { const v = e.target.value.replace(/[٠-٩]/g, d => String('٠١٢٣٤٥٦٧٨٩'.indexOf(d))).replace(/[^\d.]/g, ''); updateItem(index, 'unitPrice', v === '' ? 0 : Number(v)); }} /></div>
                      <div style={totalBox}><span style={miniLabel}>السعر الإجمالي</span><strong>{rowTotal ? rowTotal.toLocaleString('en-US') : ''} ر.س</strong></div>
                    </div>
                  );
                })}
              </div>
              <div style={grandTotal(theme)}><span>الإجمالي</span><strong>{total.toLocaleString('en-US')} ر.س</strong></div>
            </SectionCard>
...
const InvoicePreview = React.forwardRef<HTMLDivElement, { data: InvoiceData; total: number; totalWords: string; theme: string }>(({ data, total, totalWords, theme }, ref) => (
  <div ref={ref} style={invoiceStyle}>
    <div style={{ height: 7, borderRadius: 999, marginBottom: 10, background: theme }} />

    <div style={invoiceHeader}>
      <div style={craneVisual(theme)}>
        <div style={craneBadge(theme)}>TADANO</div>
        {data.logoDataUrl ? (
          <img src={data.logoDataUrl} alt="شعار" style={invoiceLogoImage} />
        ) : (
          <Truck size={90} strokeWidth={1.45} />
        )}
      </div>

      <div style={invoiceBrandBlock}>
        <div style={{ color: theme, fontWeight: 950, fontSize: 36, lineHeight: 1.1 }}>رافعات الحديثة</div>
        <div style={{ color: theme, fontWeight: 900, fontSize: 20, marginTop: 4 }}>RAFIEAT AL-HADITHA</div>
        <div style={{ ...activityRibbon, background: theme }}>{data.activity}</div>
        <div style={{ color: theme, marginTop: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, fontWeight: 900, fontSize: 16 }}>
          <MapPin size={17} />{data.companyLocation}
        </div>
      </div>
    ...
    <div style={cashTitleRow}>
      <div style={invoiceNumberBox}>
        <span style={{ color: theme, fontWeight: 900, fontSize: 15 }}>No</span>
        <strong style={{ color: theme, fontSize: 28 }}>{data.invoiceNo}</strong>
      </div>

      <div style={{ ...cashInvoiceBadge, background: theme, boxShadow: `0 8px 22px ${theme}33` }}>
        <strong style={{ fontSize: 25 }}>فاتورة نقداً</strong>
        <span style={{ fontSize: 16, fontWeight: 800 }}>CASH INVOICE</span>
        <span style={cashCheck}><CheckCircle2 size={22} /></span>
      </div>

      <div style={invoiceDateBox}>
        <div style={{ color: theme, fontWeight: 900, display: 'flex', alignItems: 'center', gap: 5 }}><CalendarDays size={17} />التاريخ</div>
        <strong>{data.date}</strong>
        <div style={{ marginTop: 7, fontWeight: 800 }}>المطلوب من السيد / السادة</div>
      </div>
    </div>

    <div style={infoCards}>
      <div style={{ ...invoiceInfoCard, borderColor: theme }}>
        <div style={{ color: theme, fontWeight: 900, marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 16 }}><UserRound size={19} />بيانات العميل</div>
        <strong style={{ fontSize: 15 }}>{data.customer || '—'}</strong>
        <div style={{ marginTop: 6, fontSize: 13 }}>{data.customerLocation || '—'}</div>
      </div>
      <div style={{ ...invoiceInfoCard, borderColor: theme }}>
        <div style={{ color: theme, fontWeight: 900, marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 16 }}><Building2 size={19} />بيانات المؤسسة</div>
        <strong style={{ fontSize: 14 }}>{data.companyArabic}</strong>
        <div dir="ltr" style={{ fontSize: 11, marginTop: 5, fontWeight: 700 }}>{data.companyEnglish}</div>
        <div style={{ marginTop: 6, fontSize: 13 }}>{data.companyLocation}</div>
      </div>
    </div>

    <div style={{ overflow: 'hidden', border: `1.5px solid ${theme}`, borderRadius: 12 }}>
      <table style={invoiceTable}>
        <thead>
          <tr>
            <th style={{ ...invoiceTh(theme), width: '7%' }}>م</th>
            <th style={{ ...invoiceTh(theme), width: '36%' }}>البيان<small style={englishSmall}>Description</small></th>
            <th style={{ ...invoiceTh(theme), width: '14%' }}>الكمية<small style={englishSmall}>Qty.</small></th>
            <th style={{ ...invoiceTh(theme), width: '20%' }}>سعر الوحدة<small style={englishSmall}>Unit Price S.R.</small></th>
            <th style={{ ...invoiceTh(theme), width: '23%' }}>السعر الإجمالي<small style={englishSmall}>Total Price S.R.</small></th>
          </tr>
        </thead>
        <tbody>
          {data.items.map((item, index) => {
            ...
            return (
              <tr key={index}>
                ...
                <td style={{ ...invoiceTd, textAlign: 'right', paddingRight: 12, fontWeight: 800 }}>{item.description || ''}</td>
                <td style={invoiceTd}>{item.qty ? String(item.qty) : ''}</td>
                <td style={invoiceTd}>{item.unitPrice ? Number(item.unitPrice).toLocaleString('en-US') : ''}</td>
                <td style={invoiceTd}>{rowTotal ? rowTotal.toLocaleString('en-US') : ''}</td>
              </tr>
            );
          })}
        </tbody>
    ...
    <div style={invoiceBottomGrid}>
      <div style={{ border: `1.5px solid ${theme}`, borderRadius: 12, overflow: 'hidden' }}>
        <SummaryRow theme={theme} label="المجموع"><strong style={{ fontSize: 18 }}>{total.toLocaleString('en-US')} ر.س</strong></SummaryRow>
        <SummaryRow theme={theme} label="المبلغ كتابة"><span>{totalWords}</span></SummaryRow>
        <SummaryRow theme={theme} label="ملاحظات"><span style={{ whiteSpace: 'pre-line', lineHeight: 1.55 }}>{data.notes}</span></SummaryRow>
        <SummaryRow theme={theme} label="حالة الدفع">
          <span style={data.paid ? invoicePaid : invoiceUnpaid}>
            <CheckCircle2 size={15} />{data.paid ? 'مدفوع' : 'غير مدفوع'}
          </span>
        </SummaryRow>
      </div>

      <div style={{ border: `1.5px solid ${theme}`, borderRadius: 12, padding: 12 }}>
        <div style={{ color: theme, fontWeight: 900, marginBottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: 16 }}><Settings size={18} />تفاصيل الفاتورة</div>
    ...
    <div style={invoiceSignatures}>
      <div><strong>توقيع المستلم</strong><div style={{ fontSize: 10, marginTop: 2 }}>Received</div><div style={{ width: 160, borderBottom: `2px dotted ${theme}`, margin: '28px auto 0' }} /></div>
      <div><strong>توقيع البائع</strong><div style={{ fontSize: 10, marginTop: 2 }}>Salesman Signature</div><div style={{ width: 160, borderBottom: `2px dotted ${theme}`, margin: '28px auto 0' }} /></div>
    </div>

    <div style={{ textAlign: 'center', fontSize: 21, fontWeight: 900, marginTop: 14, paddingBottom: 8, borderBottom: `2px solid ${theme}`, color: theme }}>• شكراً لتعاملكم معنا •</div>
  </div>
));
...
const invoiceStyle: React.CSSProperties = { width: '100%', maxWidth: 794, minHeight: 1120, margin: '0 auto', background: '#fff', color: '#101827', padding: 16, fontFamily: 'Arial, Tahoma, "Noto Sans Arabic", sans-serif', boxSizing: 'border-box', boxShadow: '0 20px 50px rgba(0,0,0,.22)', borderRadius: 16, overflow: 'hidden' };
const invoiceHeader: React.CSSProperties = { display: 'grid', gridTemplateColumns: '155px 1fr 155px', alignItems: 'center', gap: 12, minHeight: 165 };
const craneVisual = (theme: string): React.CSSProperties => ({ minHeight: 142, borderRadius: 18, background: 'linear-gradient(145deg,#ffffff,#eef3f8)', color: theme, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, overflow: 'hidden', border: `1px solid ${theme}22` });
const invoiceLogoImage: React.CSSProperties = { maxWidth: '92%', maxHeight: 112, objectFit: 'contain' };
...
const invoiceInfoCard: React.CSSProperties = { border: '1.5px solid', borderRadius: 14, padding: 12, minHeight: 110, textAlign: 'center', background: '#fff' };
const invoiceTable: React.CSSProperties = { width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' };
const invoiceTh = (theme: string): React.CSSProperties => ({ background: theme, color: '#fff', padding: '9px 4px', borderLeft: '1px solid rgba(255,255,255,.55)', fontSize: 13.5, fontWeight: 900 });
...
const invoicePaid: React.CSSProperties = { display: 'inline-flex', margin: '8px auto', alignItems: 'center', justifyContent: 'center', gap: 5, background: '#dcfce7', color: '#15803d', borderRadius: 999, padding: '5px 18px', fontWeight: 900, border: '1px solid #86efac' };
const invoiceUnpaid: React.CSSProperties = { ...invoicePaid, background: '#fee2e2', color: '#b91c1c', border: '1px solid #fecaca' };
const invoiceSignatures: React.CSSProperties = { display: 'flex', justifyContent: 'space-around', textAlign: 'center', marginTop: 20 };
const invoiceBrandBlock: React.CSSProperties = { textAlign: 'center', padding: '0 6px' };
const activityRibbon: React.CSSProperties = { color: '#fff', padding: '7px 22px', display: 'inline-block', marginTop: 8, borderRadius: 8, fontWeight: 900, fontSize: 16, minWidth: 210 };
const craneBadge = (theme: string): React.CSSProperties => ({ background: theme, color: '#fff', borderRadius: 999, padding: '3px 12px', fontSize: 11, fontWeight: 900, letterSpacing: 0.5 });
const cashTitleRow: React.CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 1.35fr 1fr', alignItems: 'center', gap: 12, margin: '2px 8px 14px' };
const invoiceNumberBox: React.CSSProperties = { display: 'grid', gap: 1, textAlign: 'left', paddingLeft: 4 };
const invoiceDateBox: React.CSSProperties = { textAlign: 'right', lineHeight: 1.55, fontSize: 13.5, paddingRight: 4 };
const cashInvoiceBadge: React.CSSProperties = { color: '#fff', borderRadius: 18, padding: '11px 16px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 92, border: '3px double rgba(255,255,255,.85)' };
const cashCheck: React.CSSProperties = { width: 30, height: 30, marginTop: 5, borderRadius: '50%', background: '#fff', color: '#138a44', display: 'flex', alignItems: 'center', justifyContent: 'center' };
