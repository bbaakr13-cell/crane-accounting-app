...
export function WorkInvoicePage() {
  ...
  async function createInvoiceCanvas() {
    ...
    drawEnglish(ctx, data.invoiceNo, 170, 430, 23);
    drawEnglish(ctx, formatDate(data.date), 884, 430, 22);

    const customerSize = data.customer.length <= 18 ? 44 : data.customer.length <= 28 ? 36 : data.customer.length <= 38 ? 30 : 26;
    drawArabic(ctx, data.customer, 730, 475, customerSize, {align:'start', maxWidth:610, minSize:24});

    const rowY = [687,727,767,807];
    data.rows.forEach((row,i) => {
      drawArabic(ctx,row.description,265,rowY[i],28,{maxWidth:390,minSize:18});
      drawEnglish(ctx,row.qty,533,rowY[i],24);
      drawEnglish(ctx,formatMoney(toNumber(row.unitPrice)),682,rowY[i],24);
      drawEnglish(ctx,formatMoney(totals[i]),874,rowY[i],24);
    });

    const wordsSize = totalWords.length <= 38 ? 34 : totalWords.length <= 50 ? 30 : totalWords.length <= 60 ? 27 : 24;
    drawArabic(ctx,totalWords,405,1238,wordsSize,{maxWidth:650,minSize:20});
    drawEnglish(ctx,formatMoney(grandTotal),902,1238,38);
    drawArabic(ctx,data.receivedBy,180,1398,20,{maxWidth:260,minSize:16});
    drawArabic(ctx,data.salesman,886,1398,20,{maxWidth:260,minSize:16});
    return canvas;
  }
...
function InvoicePreview({invoiceRef,data,totals,grandTotal,totalWords}:{invoiceRef:React.RefObject<HTMLDivElement|null>;data:InvoiceData;totals:number[];grandTotal:number;totalWords:string}) {
  const rowY=[687,727,767,807];
  const customerFontSize=data.customer.length<=18?44:data.customer.length<=28?36:data.customer.length<=38?30:26;
  const totalWordsFontSize=totalWords.length<=38?34:totalWords.length<=50?30:totalWords.length<=60?27:24;
  return <div className="w-full"><div ref={invoiceRef} className="relative w-full bg-white overflow-hidden" style={{aspectRatio:`${TEMPLATE_WIDTH} / ${TEMPLATE_HEIGHT}`}}>
    ...
    <svg viewBox={`0 0 ${TEMPLATE_WIDTH} ${TEMPLATE_HEIGHT}`} preserveAspectRatio="none" className="absolute inset-0 w-full h-full">
      <SvgEnglishText x={170} y={430} size={23} weight={900}>{data.invoiceNo}</SvgEnglishText>
      <SvgEnglishText x={884} y={430} size={22} weight={900}>{formatDate(data.date)}</SvgEnglishText>
      <text x={730} y={475} fill="#000" fontSize={customerFontSize} fontWeight={400} textAnchor="start" dominantBaseline="middle" direction="rtl" unicodeBidi="plaintext" style={{fontFamily:`'${ARABIC_FONT_NAME}'`,fontWeight:400,fontStyle:'normal'}}>{data.customer}</text>
      {data.rows.map((row,i)=><React.Fragment key={i}>
        <SvgArabicText x={265} y={rowY[i]} size={28}>{row.description}</SvgArabicText>
        <SvgEnglishText x={533} y={rowY[i]} size={24} weight={900}>{row.qty}</SvgEnglishText>
        <SvgEnglishText x={682} y={rowY[i]} size={24} weight={900}>{formatMoney(toNumber(row.unitPrice))}</SvgEnglishText>
        <SvgEnglishText x={874} y={rowY[i]} size={24} weight={900}>{formatMoney(totals[i])}</SvgEnglishText>
      </React.Fragment>)}
      <SvgArabicText x={405} y={1238} size={totalWordsFontSize}>{totalWords}</SvgArabicText>
      <SvgEnglishText x={902} y={1238} size={38} weight={900}>{formatMoney(grandTotal)}</SvgEnglishText>
      <SvgArabicText x={180} y={1398} size={20}>{data.receivedBy}</SvgArabicText>
      <SvgArabicText x={886} y={1398} size={20}>{data.salesman}</SvgArabicText>
    </svg>
  </div></div>;
}
