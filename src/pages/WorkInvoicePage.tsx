...
export function WorkInvoicePage() {
  ...
  async function createInvoiceCanvas() {
    ...
    drawArabic(ctx, data.customer, 730, 493, customerSize, {align:'start', maxWidth:610, minSize:24});
    ...
    drawArabic(ctx,totalWords,405,1262,wordsSize,{maxWidth:650,minSize:20});
    drawEnglish(ctx,formatMoney(grandTotal),902,1262,38);
...
function InvoicePreview({invoiceRef,data,totals,grandTotal,totalWords}:{invoiceRef:React.RefObject<HTMLDivElement|null>;data:InvoiceData;totals:number[];grandTotal:number;totalWords:string}) {
  ...
  return <div className="w-full"><div ref={invoiceRef} className="relative w-full bg-white overflow-hidden" style={{aspectRatio:`${TEMPLATE_WIDTH} / ${TEMPLATE_HEIGHT}`}}>
    ...
    <svg viewBox={`0 0 ${TEMPLATE_WIDTH} ${TEMPLATE_HEIGHT}`} preserveAspectRatio="none" className="absolute inset-0 w-full h-full">
      ...
      <text x={730} y={493} fill="#000" fontSize={customerFontSize} fontWeight={400} textAnchor="start" dominantBaseline="middle" direction="rtl" unicodeBidi="plaintext" style={{fontFamily:`'${ARABIC_FONT_NAME}'`,fontWeight:400,fontStyle:'normal'}}>{data.customer}</text>
      ...
      <SvgArabicText x={405} y={1262} size={totalWordsFontSize}>{totalWords}</SvgArabicText>
      <SvgEnglishText x={902} y={1262} size={38} weight={900}>{formatMoney(grandTotal)}</SvgEnglishText>
