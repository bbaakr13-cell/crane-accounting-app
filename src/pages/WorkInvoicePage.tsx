...
export function WorkInvoicePage() {
  ...
  async function createInvoiceCanvas() {
    ...
    drawArabic(ctx, data.customer, 730, 452, customerSize, {align:'start', maxWidth:610, minSize:24});
    ...
    const wordsSize = totalWords.length <= 38 ? 30 : totalWords.length <= 50 ? 27 : totalWords.length <= 60 ? 24 : 21;
    drawArabic(ctx,totalWords,330,1302,wordsSize,{maxWidth:500,minSize:18});
    drawEnglish(ctx,formatMoney(grandTotal),902,1302,32);
...
function InvoicePreview({invoiceRef,data,totals,grandTotal,totalWords}:{invoiceRef:React.RefObject<HTMLDivElement|null>;data:InvoiceData;totals:number[];grandTotal:number;totalWords:string}) {
  ...
  const totalWordsFontSize=totalWords.length<=38?30:totalWords.length<=50?27:totalWords.length<=60?24:21;
  return <div className="w-full"><div ref={invoiceRef} className="relative w-full bg-white overflow-hidden" style={{aspectRatio:`${TEMPLATE_WIDTH} / ${TEMPLATE_HEIGHT}`}}>
    ...
    <svg viewBox={`0 0 ${TEMPLATE_WIDTH} ${TEMPLATE_HEIGHT}`} preserveAspectRatio="none" className="absolute inset-0 w-full h-full">
      ...
      <text x={730} y={452} fill="#000" fontSize={customerFontSize} fontWeight={400} textAnchor="start" dominantBaseline="middle" direction="rtl" unicodeBidi="plaintext" style={{fontFamily:`'${ARABIC_FONT_NAME}'`,fontWeight:400,fontStyle:'normal'}}>{data.customer}</text>
      ...
      <SvgArabicText x={330} y={1302} size={totalWordsFontSize}>{totalWords}</SvgArabicText>
      <SvgEnglishText x={902} y={1302} size={32} weight={900}>{formatMoney(grandTotal)}</SvgEnglishText>
