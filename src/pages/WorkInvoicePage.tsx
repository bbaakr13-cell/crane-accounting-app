...
export function WorkInvoicePage() {
  ...
  async function createInvoiceCanvas() {
    ...
    drawArabic(ctx, data.customer, 730, 493, customerSize, {align:'right', maxWidth:610, minSize:20});

    const rowY = [687,727,767,807];
    data.rows.forEach((row,i) => {
      drawArabic(ctx,row.description,265,rowY[i],28,{maxWidth:430,minSize:18});
    ...
    drawArabic(ctx,totalWords,405,1262,wordsSize,{maxWidth:560,minSize:18});
    drawEnglish(ctx,formatMoney(grandTotal),902,1262,38);
    drawArabic(ctx,data.receivedBy,180,1398,20,{maxWidth:260,minSize:14});
    drawArabic(ctx,data.salesman,886,1398,20,{maxWidth:260,minSize:14});
    return canvas;
  }
