...
export function WorkInvoicePage() {
  ...
  async function createInvoiceCanvas() {
    ...
    drawEnglish(ctx, data.invoiceNo, 142, 552, 18);
    drawEnglish(ctx, formatDate(data.date), 852, 552, 18);

    const customerSize = data.customer.length <= 18 ? 30 : data.customer.length <= 28 ? 27 : data.customer.length <= 38 ? 24 : 21;
    drawArabic(ctx, data.customer, 520, 590, customerSize, {align:'center', maxWidth:560, minSize:18});

    const rowY = [708,748,788,828];
    data.rows.forEach((row,i) => {
      drawArabic(ctx,row.description,275,rowY[i],24,{maxWidth:390,minSize:17});
      drawEnglish(ctx,row.qty,530,rowY[i],21);
      drawEnglish(ctx,formatMoney(toNumber(row.unitPrice)),681,rowY[i],21);
      drawEnglish(ctx,formatMoney(totals[i]),875,rowY[i],21);
    });

    const wordsSize = totalWords.length <= 38 ? 23 : totalWords.length <= 50 ? 21 : totalWords.length <= 60 ? 19 : 17;
    drawArabic(ctx,totalWords,330,1248,wordsSize,{maxWidth:500,minSize:16});
    drawEnglish(ctx,formatMoney(grandTotal),904,1248,29);
    drawArabic(ctx,data.receivedBy,180,1342,18,{maxWidth:260,minSize:14});
    drawArabic(ctx,data.salesman,886,1342,18,{maxWidth:260,minSize:14});
    return canvas;
  }
...
function InvoicePreview({invoiceRef,data,totals,grandTotal,totalWords}:{invoiceRef:React.RefObject<HTMLDivElement|null>;data:InvoiceData;totals:number[];grandTotal:number;totalWords:string}) {
  ...
  return <div className="w-full"><div ref={invoiceRef} className="relative w-full bg-white overflow-hidden" style={{aspectRatio:`${TEMPLATE_WIDTH} / ${TEMPLATE_HEIGHT}`}}>
    ...
    <svg viewBox={`0 0 ${TEMPLATE_WIDTH} ${TEMPLATE_HEIGHT}`} preserveAspectRatio="none" className="absolute inset-0 w-full h-full">
      <SvgEnglishText x={142} y={552} size={18} weight={900}>{data.invoiceNo}</SvgEnglishText>
      <SvgEnglishText x={852} y={552} size={18} weight={900}>{formatDate(data.date)}</SvgEnglishText>
      <text x={520} y={590} fill="#000" fontSize={customerFontSize} fontWeight={400} textAnchor="middle" dominantBaseline="middle" direction="rtl" unicodeBidi="plaintext" style={{fontFamily:`'${ARABIC_FONT_NAME}'`,fontWeight:400,fontStyle:'normal'}}>{data.customer}</text>
      {data.rows.map((row,i)=><React.Fragment key={i}>
        <SvgArabicText x={275} y={rowY[i]} size={24}>{row.description}</SvgArabicText>
        <SvgEnglishText x={530} y={rowY[i]} size={21} weight={900}>{row.qty}</SvgEnglishText>
        <SvgEnglishText x={681} y={rowY[i]} size={21} weight={900}>{formatMoney(toNumber(row.unitPrice))}</SvgEnglishText>
        <SvgEnglishText x={875} y={rowY[i]} size={21} weight={900}>{formatMoney(totals[i])}</SvgEnglishText>
      </React.Fragment>)}
      <SvgArabicText x={330} y={1248} size={totalWordsFontSize}>{totalWords}</SvgArabicText>
      <SvgEnglishText x={904} y={1248} size={29} weight={900}>{formatMoney(grandTotal)}</SvgEnglishText>
      <SvgArabicText x={180} y={1342} size={18}>{data.receivedBy}</SvgArabicText>
      <SvgArabicText x={886} y={1342} size={18}>{data.salesman}</SvgArabicText>
    </svg>
  </div></div>;
}
