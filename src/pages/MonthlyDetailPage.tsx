...
export function MonthlyDetailPage() {
  ...
  useEffect(() => {
    ...
    // لا نسمح لطلب المعدات أن يعلّق الصفحة على "جاري التحميل" إلى ما لا نهاية.
    // إذا لم يرد مصدر البيانات خلال 5 ثوانٍ، نكمل فتح الحساب بالمعدة القادمة من الرابط.
    const equipmentTimeout = new Promise<Equipment[]>((resolve) => {
      window.setTimeout(() => resolve([]), 5000);
    });

    Promise.race([fetchEquipment(), equipmentTimeout])
      .then(list => {
        if (!alive) return;
        ...
        setEquipmentId(current => {
          ...
          // لو تعذر تحميل قائمة المعدات، احتفظ بمعرّف المعدة الموجود في الرابط
          // حتى تظل بيانات الحساب الشهري القديمة قابلة للقراءة من localStorage.
          return safe.length ? String(safe[0].id) : (id || current || '');
        });
      })
      .catch(error => {
        ...
        <div style={{background:'#0b1527',border:'1px solid #1d2d47',borderRadius:18,padding:14,marginBottom:18,display:'grid',gap:10}}>
          <label>
            ...
            <select value={equipmentId} onChange={e=>setEquipmentId(e.target.value)} style={selectStyle} disabled={equipmentLoading || equipmentList.length===0}>
              {equipmentLoading ? (
                <option value={equipmentId}>جاري تحميل المعدات...</option>
              ) : equipmentList.length===0 ? (
                <option value={equipmentId}>{equipmentId ? `المعدة ${equipmentId}` : 'لا توجد معدات'}</option>
              ) : equipmentList.map(x=><option key={x.id} value={String(x.id)}>{formatEquipmentName(x.name)}</option>)}
            </select>
          </label>
