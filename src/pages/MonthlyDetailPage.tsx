...
const MONTHLY_HEADER_URL = '/monthly-header-small.png';
const MONTHLY_FOOTER_URL = '/monthly-footer-small.png';
const PDF_WIDTH = 794;
...
export function MonthlyDetailPage() {
  ...
  async function createPdfCanvas() {
    ...
    roundedRect(ctx, 643, 118, 48, 22, 6);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    drawArabic(ctx, monthNames[month], 667, 129, 10.5, TEXT);

    roundedRect(ctx, 708, 118, 48, 22, 6);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    drawEnglish(ctx, String(year), 732, 129, 10, TEXT, 800);

    // القيم داخل الخانات السفلية للهيدر
    const headerValueY = 180;

    roundedRect(ctx, 28, 165, 168, 18, 5);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    drawArabic(
      ctx,
