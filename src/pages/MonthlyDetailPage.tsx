...
export function MonthlyDetailPage() {
  ...
  function drawInfoBox(
    ...
    drawArabic(ctx, label, x + width / 2, y + 22, 25, '#0b3b82');
    const valueSize = fitFontSize(ctx, value, width - 16, 30, 18, HACEN_FONT_NAME);
    drawArabic(ctx, value, x + width / 2, y + 53, valueSize, '#111827');
  }
  ...
  async function createPdfCanvas() {
    ...
    drawEnglish(ctx, 'BAKR PRO', PDF_WIDTH / 2, 28, 29, '#0b3b82', 900);
    drawArabic(ctx, 'كشف الحساب الشهري', PDF_WIDTH / 2, 59, 30, '#102f61');
    drawArabic(ctx, 'تقرير شامل للأعمال والمشاوير والمصاريف', PDF_WIDTH / 2, 87, 22, '#475569');

    const marginX = 18;
    const contentWidth = PDF_WIDTH - marginX * 2;
    const infoY = 108;
    const infoHeight = 74;
    const equipmentWidth = contentWidth * (1.4 / 3.4);
    const monthWidth = contentWidth * (1 / 3.4);
    const yearWidth = contentWidth - equipmentWidth - monthWidth;
    const equipmentX = PDF_WIDTH - marginX - equipmentWidth;
    const monthX = equipmentX - monthWidth;
    const yearX = monthX - yearWidth;

    drawInfoBox(ctx, equipmentX, infoY, equipmentWidth, infoHeight, 'المعدة', displayEquipmentName);
    drawInfoBox(ctx, monthX, infoY, monthWidth, infoHeight, 'الشهر', monthNames[month]);
    drawInfoBox(ctx, yearX, infoY, yearWidth, infoHeight, 'السنة', String(year));
    ...
    const columns = [
      ...
      { key: 'work', label: 'نوع العمل', ratio: 0.15 },
      { key: 'location', label: 'موقع العمل', ratio: 0.16 },
      { key: 'trip', label: 'سعر المشوار', ratio: 0.14 },
      { key: 'expense', label: 'بيان المصروف', ratio: 0.18 },
      { key: 'amount', label: 'المبلغ', ratio: 0.13 },
      { key: 'notes', label: 'ملاحظات', ratio: 0.17 },
    ];
    ...
    columnRects.forEach((column) => {
      ...
      const size = fitFontSize(ctx, column.label, column.width - 8, 21, 15, HACEN_FONT_NAME);
    ...
    for (let day = 1; day <= 31; day += 1) {
      ...
      const values: Record<string, string> = {
        ...
        trip: row.tripPrice > 0 ? `${row.tripPrice.toLocaleString('en-US')} ر.س` : '',
        expense: expenseDescription,
        amount: combinedExpense > 0 ? `${combinedExpense.toLocaleString('en-US')} ر.س` : '',
        notes: combinedNotes,
      };
      ...
      columnRects.forEach((column) => {
        ...
        if (!text) return;
        const size = fitFontSize(ctx, text, column.width - 8, 20, 11, HACEN_FONT_NAME);
        drawArabic(
          ctx,
          text,
          ...
          rowY + rowHeight / 2,
          size,
          column.key === 'amount' && combinedExpense > 0 ? '#dc2626' : '#111827'
        );
      });
    }
    ...
    const summaryItems = [
      { label: 'إجمالي الدخل', value: `${totals.income.toLocaleString('en-US')} ر.س`, border: '#16a34a', color: '#15803d' },
      { label: 'إجمالي المصروفات', value: `${totalExpense.toLocaleString('en-US')} ر.س`, border: '#ef4444', color: '#dc2626' },
      { label: 'صافي الشهر', value: `${net.toLocaleString('en-US')} ر.س`, border: '#2563eb', color: net >= 0 ? '#2563eb' : '#dc2626' },
      { label: 'أيام مسجلة', value: String(totals.registeredDays), border: '#f59e0b', color: '#111827' },
      { label: 'المشاوير', value: String(totals.trips), border: '#7c3aed', color: '#111827' },
    ];

    summaryItems.forEach((item, index) => {
      const x = PDF_WIDTH - marginX - summaryWidth - index * (summaryWidth + summaryGap);
      roundedRect(ctx, x, summaryY, summaryWidth, summaryHeight, 7);
      ctx.fillStyle = '#ffffff';
      ctx.fill();
      ctx.strokeStyle = item.border;
      ctx.lineWidth = 1;
      ctx.stroke();
      const labelSize = fitFontSize(ctx, item.label, summaryWidth - 10, 21, 14, HACEN_FONT_NAME);
      drawArabic(ctx, item.label, x + summaryWidth / 2, summaryY + 22, labelSize, '#111827');
      const valueSize = fitFontSize(ctx, item.value, summaryWidth - 10, 22, 14, HACEN_FONT_NAME);
      drawArabic(ctx, item.value, x + summaryWidth / 2, summaryY + 49, valueSize, item.color);
    });

    let nextY = summaryY + summaryHeight + 6;

    if (totals.linkedExpense > 0) {
      const redHeight = 36;
      roundedRect(ctx, marginX, nextY, contentWidth, redHeight, 6);
      ctx.fillStyle = '#fff1f2';
      ctx.fill();
      ctx.strokeStyle = '#fecaca';
      ctx.lineWidth = 1;
      ctx.stroke();
      const redText = `مصاريف السواقين والمعدات: ${totals.linkedExpense.toLocaleString('en-US')} ر.س`;
      const redSize = fitFontSize(ctx, redText, contentWidth - 20, 24, 18, HACEN_FONT_NAME);
      drawArabic(ctx, redText, PDF_WIDTH / 2, nextY + redHeight / 2, redSize, '#dc2626');
      nextY += redHeight + 6;
    }

    const footerHeight = 34;
    roundedRect(ctx, marginX, nextY, contentWidth, footerHeight, 4);
    ctx.fillStyle = '#073b7a';
    ctx.fill();
    drawArabic(ctx, 'تم إعداد هذا الكشف بواسطة BAKR PRO', PDF_WIDTH / 2, nextY + footerHeight / 2, 20, '#ffffff');
