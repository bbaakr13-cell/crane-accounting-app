...
export function MonthlyDetailPage() {
  ...
  async function analyzeMonthlyImage() {
    ...
    try {
      ...
      const clean: DayRow[] = source
        .map((r: any): DayRow => {
          const parsedDay = Number(
            normalizeArabicNumbers(String(r.day ?? ''))
          );
          const parsedTripPrice = Number(
            normalizeArabicNumbers(
              String(r.tripPrice ?? r.amount ?? 0)
            )
          );
          const parsedExpense = Number(
            normalizeArabicNumbers(
              String(r.expenseAmount ?? 0)
            )
          );
          ...
          return {
            day: Math.max(
              1,
              Math.min(daysInMonth, parsedDay || 1)
            ),
            workType: String(r.workType || '').trim(),
            tripType: String(
              r.tripType || r.location || ''
            ).trim(),
            tripPrice: Number.isFinite(parsedTripPrice)
              ? parsedTripPrice
              : 0,
            expenseType: String(r.expenseType || '').trim(),
        ...
        .filter(
          (r: DayRow, index: number, array: DayRow[]) =>
            r.day >= 1 &&
            r.day <= daysInMonth &&
            array.findIndex(x => x.day === r.day) === index
        )
