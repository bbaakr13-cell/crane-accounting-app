...
export function MonthlyDetailPage() {
  ...
  function drawInfoBox(ctx: CanvasRenderingContext2D, x: number, y: number, width: number, height: number, label: string, value: string) {
    ...
    ctx.lineWidth = 1;
    ctx.stroke();
    drawArabic(ctx, label, x + width / 2, y + 15, 14, '#0b3b82');
    const valueSize = fitFontSize(ctx, value, width - 14, 17, 11.5, HACEN_FONT_NAME);
    drawArabic(ctx, value, x + width / 2, y + 34, valueSize, '#111827');
  }
  ...
  async function createPdfCanvas() {
    ...
    // Header - compact professional monthly statement
    const headerH = 120;
    const headerGradient = ctx.createLinearGradient(0, 0, PDF_WIDTH, 0);
    headerGradient.addColorStop(0, '#061827');
    headerGradient.addColorStop(0.68, '#082c5f');
    headerGradient.addColorStop(1, '#0b3b82');
    ctx.fillStyle = headerGradient;
    ctx.fillRect(0, 0, PDF_WIDTH, headerH);

    ctx.fillStyle = '#f4b62c';
    ctx.fillRect(0, 0, 8, headerH);
    ...
    ctx.moveTo(505, 0);
    ctx.lineTo(630, 0);
    ctx.lineTo(560, headerH);
    ctx.lineTo(435, headerH);
    ctx.closePath();
    ctx.fill();
    ctx.restore();

    drawEnglish(ctx, 'BAKR PRO', 145, 36, 30, '#ffffff', 900);
    drawArabic(ctx, 'إدارة حسابات الكرينات', 145, 63, 15, '#ffffff');
    drawEnglish(ctx, 'CRANE ACCOUNTING MANAGEMENT', 145, 84, 9, '#cfe6ff', 700);
    drawArabic(ctx, 'دقة في الحساب .. وضوح في الإدارة', 145, 103, 10.5, '#f4b62c');

    drawArabic(ctx, 'كشف الحساب الشهري', 590, 37, 27, '#ffffff');
    drawEnglish(ctx, 'MONTHLY ACCOUNT STATEMENT', 590, 65, 11, '#dbeafe', 700);
    drawArabic(ctx, 'تنظيم أوضح .. متابعة أدق', 590, 88, 11, '#dbeafe');

    roundedRect(ctx, 520, 96, 140, 19, 6);
    ctx.fillStyle = '#ffffff16';
    ctx.fill();
    drawArabic(ctx, `${monthNames[month]} ${year}`, 590, 105, 11, '#f4b62c');
    ...
    drawArabic(ctx, 'ملف الحساب الشهري', PDF_WIDTH / 2, titleY + 18, 24, '#ffffff');
    drawEnglish(ctx, 'MONTHLY ACCOUNT STATEMENT', PDF_WIDTH / 2, titleY + 36, 10.5, '#dbeafe', 700);

    roundedRect(ctx, PDF_WIDTH - 168, titleY + 7, 138, 34, 8);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    drawArabic(ctx, 'الشهر', PDF_WIDTH - 136, titleY + 17, 9.5, NAVY);
    drawArabic(ctx, monthNames[month], PDF_WIDTH - 136, titleY + 31, 12, TEXT);
    drawArabic(ctx, 'السنة', PDF_WIDTH - 72, titleY + 17, 9.5, NAVY);
    drawEnglish(ctx, String(year), PDF_WIDTH - 72, titleY + 31, 11.5, TEXT, 800);
    ...
    columnRects.forEach((column) => {
      ...
      const size = fitFontSize(ctx, column.label, column.width - 10, 18.5, 13, HACEN_FONT_NAME);
    ...
    summaryItems.forEach((item, index) => {
      ...
      roundedRect(ctx, x, summaryY, sumWidth, 60, 10);
      ctx.fillStyle = item.bg;
      ctx.fill();
      ctx.strokeStyle = `${item.color}55`;
      ctx.lineWidth = 1;
      ctx.stroke();
      drawArabic(ctx, item.label, x + sumWidth / 2, summaryY + 19, 13, '#123761');
    ...
    footerGradient.addColorStop(0, '#061827');
    footerGradient.addColorStop(1, '#082c5f');
    ...
    drawEnglish(ctx, 'BAKR PRO', 105, footerY + 22, 16, '#ffffff', 900);
    drawEnglish(ctx, 'BAKR ALMASBHI © 2026', 105, footerY + 40, 8.5, '#cfe6ff', 700);

    drawEnglish(ctx, '0558995962', PDF_WIDTH / 2, footerY + 23, 19, '#f4b62c', 900);
    drawArabic(ctx, 'خميس مشيط - أبها', PDF_WIDTH / 2, footerY + 42, 10, '#ffffff');

    drawArabic(ctx, 'معك في كل مشروع', PDF_WIDTH - 105, footerY + 20, 12, '#ffffff');
    drawEnglish(ctx, 'All Rights Reserved.', PDF_WIDTH - 105, footerY + 40, 8.5, '#cfe6ff', 700);
