...
const MONTHLY_FOOTER_URL = `${PUBLIC_BASE}monthly-footer.png`;
const PDF_WIDTH = 794;
  ...
  return new Promise((resolve, reject) => {
    ...
    const tryNext = () => {
      if (index >= candidates.length) {
        reject(new Error(`تعذر تحميل الصورة: ${src}`));
        return;
      }
...
export function MonthlyDetailPage() {
  ...
  async function ensureHacenFont() {
    ...
    let response = await fetch(HACEN_FONT_URL, { cache: 'no-store' });

    if (!response.ok) {
      const fallbackFontUrl = HACEN_FONT_URL.startsWith('/')
        ? `.${HACEN_FONT_URL}`
        : `./hacen-egypt.ttf`;

      response = await fetch(fallbackFontUrl, { cache: 'no-store' });
    }
  ...
  async function createPdfCanvas() {
    ...
    // قيم الشهر والسنة داخل خانات الهيدر الأصلية
    // لا نرسم مستطيلات بيضاء فوق التصميم؛ نضع القيم فقط.
    drawArabic(ctx, monthNames[month], 636, 202, 11.5, TEXT);
    drawEnglish(ctx, String(year), 737, 202, 11, TEXT, 800);

    // القيم داخل خانات الهيدر الأصلية
    // الهيدر يبقى كما هو، فقط نضبط أماكن البيانات داخل الخانات.
    const headerValueY = 284;
    ...
    drawArabic(
      ...
      112,
      headerValueY,
      fitFontSize(ctx, displayEquipmentName, 150, 12, 8, HACEN_FONT_NAME),
      TEXT
    );
    ...
    // خانة السائق / المشغل: متروكة فارغة حالياً.
    // خانة الملاحظات: متروكة فارغة حالياً.
    ...
    // =========================================================
    // 3) ملخص صغير أسفل الجدول — بدون الفوتر الكبير
    // =========================================================
    const summaryY = footerY + 8;
    const summaryGap = 8;
    const summaryWidth = (PDF_WIDTH - 36 - summaryGap * 3) / 4;
    const summaryH = 54;

    const summaryItems = [
      {
        label: 'إجمالي المشاوير',
    ...
    // سطر حقوق خفيف جداً في الأسفل
    const rightsY = summaryY + summaryH + 22;
