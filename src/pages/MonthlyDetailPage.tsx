...
export function MonthlyDetailPage() {
  ...
  async function createPdfCanvas() {
    ...
    roundedRect(ctx, 643, 166, 48, 22, 6);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    drawArabic(ctx, monthNames[month], 667, 177, 10.5, TEXT);

    roundedRect(ctx, 708, 166, 48, 22, 6);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    drawEnglish(ctx, String(year), 732, 177, 10, TEXT, 800);

    // القيم داخل الخانات السفلية للهيدر
    const headerValueY = 278;

    roundedRect(ctx, 28, 265, 168, 20, 5);
    ctx.fillStyle = '#ffffff';
    ctx.fill();
    drawArabic(
      ctx,
