...
export function ExpensesPage() {
  ...
  function makePdf() {
    ...
    const pages = Math.max(1, Math.ceil(monthlyRows.length / rowsPerPage));
    ...
    const issueDate = `${issue.getFullYear()}/${String(issue.getMonth() + 1).padStart(2, '0')}/${String(issue.getDate()).padStart(2, '0')}`;
    const issueTime = `${String(issue.getHours()).padStart(2, '0')}:${String(issue.getMinutes()).padStart(2, '0')}`;
    ...
    for (let page = 0; page < pages; page += 1) {
      ...
      grad.addColorStop(0, '#061523');
      grad.addColorStop(0.55, '#0a2238');
      grad.addColorStop(1, '#07131f');
      ctx.fillStyle = grad;
      ctx.fillRect(0, 0, canvas.width, 280);
      ctx.fillStyle = '#f2b01e';
      ctx.fillRect(0, 272, canvas.width, 8);
      ...
      drawText('BAKR PRO', 1150, 55, 42, '#fbbf24', 900);
      drawText('إدارة حسابات الكرينات', 1150, 102, 24, '#ffffff', 700);
      drawText('كشف حساب المصاريف الشهري', 620, 78, 46, '#ffffff', 900, 'center');
      ctx.save();
      ctx.direction = 'ltr';
      ...
      ctx.fillText('MONTHLY EXPENSES STATEMENT', 620, 126);
      ctx.restore();
      drawText('دقة في الحساب • وضوح في الإدارة • نجاح في العمل', 620, 168, 20, '#dbe6f2', 500, 'center');
      drawText(`الشهر: ${monthNames[month]} ${year}`, 1160, 214, 22, '#ffffff', 700);
      drawText(`تاريخ الإصدار: ${issueDate}`, 650, 214, 19, '#cbd5e1', 500);
      drawText(`الوقت: ${issueTime}`, 380, 214, 19, '#cbd5e1', 500);

      // Summary cards
      const cards = [
        { x: 40, label: 'مصاريف عامة', value: totals.general, bg: '#f7f0ff', accent: '#8b5cf6' },
        { x: 335, label: 'مصاريف السائقين', value: totals.drivers, bg: '#eaf9f3', accent: '#10b981' },
        { x: 630, label: 'مصاريف الكرينات', value: totals.equipment, bg: '#fff7df', accent: '#d49a16' },
        { x: 925, label: 'إجمالي المصاريف', value: totals.total, bg: '#eaf3ff', accent: '#2563eb' },
      ];

      cards.forEach((card) => {
        drawRoundedRect(card.x, 315, 275, 125, 18, card.bg, '#dbe5ef');
        drawText(card.label, card.x + 245, 348, 18, '#334155', 700);
        drawText(money(card.value), card.x + 245, 394, 31, card.accent, 900);
      });
      ...
      const headerH = 56;
      const rowH = 58;
      drawRoundedRect(tableX, tableY, tableW, headerH, 12, '#082036');

      const cols = [
        { key: 'date', label: 'التاريخ', w: 170 },
        { key: 'category', label: 'نوع المصروف', w: 195 },
        { key: 'linked', label: 'الكرين / السائق', w: 225 },
        { key: 'amount', label: 'المبلغ', w: 145 },
        { key: 'payment', label: 'طريقة الدفع', w: 145 },
        { key: 'location', label: 'الموقع / الجهة', w: 145 },
        { key: 'notes', label: 'الملاحظات', w: 135 },
      ];

      let cx = tableX + tableW;
      cols.forEach((col) => {
        cx -= col.w;
        drawText(col.label, cx + col.w - 12, tableY + headerH / 2, 15, '#ffffff', 700);
      });

      const rows = monthlyRows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
      rows.forEach((row, i) => {
        const item = row.item;
        const y = tableY + headerH + i * rowH;
        ctx.fillStyle = i % 2 === 0 ? '#ffffff' : '#f8fbff';
        ...
        const values = [
          `${year}/${String(month + 1).padStart(2, '0')}/${String(row.day).padStart(2, '0')}`,
          item?.category || '—',
          item ? linkedLabel(item) : '—',
        ...
        cols.forEach((col, idx) => {
          xCursor -= col.w;
          const max = idx === 6 ? 15 : idx === 5 ? 16 : idx === 2 ? 18 : 22;
          const txt = values[idx].length > max ? `${values[idx].slice(0, max)}…` : values[idx];
          drawText(txt, xCursor + col.w - 12, y + rowH / 2, 14, item ? '#1e293b' : '#94a3b8', idx === 3 && item ? 800 : 500);
        });
      });

      const afterTableY = tableY + headerH + rowsPerPage * rowH + 34;

      if (page === pages - 1) {
        // Bottom analysis section
        drawRoundedRect(40, afterTableY, 560, 205, 18, '#f8fafc', '#dbe5ef');
        drawText('ملخص المصاريف', 565, afterTableY + 34, 20, '#0f172a', 800);
        drawText(`إجمالي العمليات: ${totals.count}`, 565, afterTableY + 76, 16, '#475569', 600);
        drawText(`مصاريف الكرينات: ${money(totals.equipment)}`, 565, afterTableY + 112, 16, '#b7790a', 700);
        drawText(`مصاريف السائقين: ${money(totals.drivers)}`, 565, afterTableY + 148, 16, '#0f8a65', 700);
        drawText(`مصاريف عامة: ${money(totals.general)}`, 565, afterTableY + 184, 16, '#7c3aed', 700);

        drawRoundedRect(620, afterTableY, 580, 205, 18, '#081c31', '#163754');
        drawText('ملاحظات', 1165, afterTableY + 34, 20, '#ffffff', 800);
        drawText('• هذا الكشف تم إنشاؤه تلقائياً من نظام BAKR PRO.', 1165, afterTableY + 78, 15, '#dbe6f2', 500);
        drawText('• يشمل جميع المصاريف المسجلة خلال الشهر المحدد.', 1165, afterTableY + 112, 15, '#dbe6f2', 500);
        drawText('• يمكن حفظ الكشف أو مشاركته مع الجهات المعنية.', 1165, afterTableY + 146, 15, '#dbe6f2', 500);
        drawText('• البيانات مرتبطة بالكرينات والسائقين تلقائياً.', 1165, afterTableY + 180, 15, '#dbe6f2', 500);
      }

      // Footer
      const footerY = 1640;
      ctx.fillStyle = '#061523';
      ctx.fillRect(0, footerY, canvas.width, 114);
      ctx.fillStyle = '#f2b01e';
      ctx.fillRect(0, footerY, canvas.width, 5);
      drawText('BAKR PRO', 1160, footerY + 40, 24, '#fbbf24', 900);
      drawText('إدارة حسابات الكرينات', 1160, footerY + 74, 16, '#ffffff', 600);
      drawText('خميس مشيط - أبها', 650, footerY + 46, 16, '#e2e8f0', 600, 'center');
      drawText('0558995962', 650, footerY + 78, 18, '#ffffff', 800, 'center');
      drawText(`صفحة ${page + 1} من ${pages}`, 90, footerY + 58, 16, '#cbd5e1', 600, 'left');
